// scripts/fix-failed-order.ts
// Script to manually complete a failed order by updating MongoDB with AI evaluation
...
async function fixFailedOrder(documentId: string): Promise<void> {
  ...
  try {
    ...
    const collection = db.collection<MongoDocument>('bewertungen');
...
async function main(): Promise<void> {
  ...
  if (!documentId) {
    console.error('❌ Usage: npm run fix-order <document-id>');
    console.error('Example: npm run fix-order 68963bc9818a940a01833301');
    process.exit(1);
  }
